...
func TestIdentityProviderPasswordPolicyCreateReadUpdateAndDeleteReportInvalidStateData(t *testing.T) {
	t.Parallel()
	...
	raw := tftypes.NewValue(
		tftypes.Object{AttributeTypes: map[string]tftypes.Type{
			...
			"identity_provider_id": tftypes.String,
			"password_policy_id":   tftypes.String,
		}},
		map[string]tftypes.Value{
			"id":                   tftypes.NewValue(tftypes.String, "domain-123/idp-123"),
			"domain_id":            tftypes.NewValue(tftypes.Number, 123),
			"identity_provider_id": tftypes.NewValue(tftypes.String, "idp-123"),
			"password_policy_id":   tftypes.NewValue(tftypes.String, "policy-123"),
		},
	)
