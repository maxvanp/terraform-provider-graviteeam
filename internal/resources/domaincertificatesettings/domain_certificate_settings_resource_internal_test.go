...
func TestDomainCertificateSettingsConfigureAcceptsClient(t *testing.T) {
	t.Parallel()
...
func TestDomainCertificateSettingsCreateReadUpdateAndDeleteReportInvalidStateData(t *testing.T) {
	t.Parallel()
	...
	raw := tftypes.NewValue(
		tftypes.Object{AttributeTypes: map[string]tftypes.Type{
			...
			"fallback_certificate_id": tftypes.String,
		}},
		map[string]tftypes.Value{
			"id":                      tftypes.NewValue(tftypes.String, "domain-123"),
			"domain_id":               tftypes.NewValue(tftypes.Number, 123),
			"fallback_certificate_id": tftypes.NewValue(tftypes.String, "cert-123"),
		},
	)
