...
func TestMetadata(t *testing.T) {
	...
	NewOrgMemberResource().Metadata(context.Background(), resource.MetadataRequest{
		ProviderTypeName: "graviteeam",
	}, &resp)

	if got, want := resp.TypeName, "graviteeam_org_member"; got != want {
...
func TestSchemaAttributes(t *testing.T) {
	...
	NewOrgMemberResource().Schema(context.Background(), resource.SchemaRequest{}, &resp)
...
func TestConfigureRejectsUnexpectedProviderData(t *testing.T) {
	...
	(&OrgMemberResource{}).Configure(context.Background(), resource.ConfigureRequest{
...
func TestOrgMemberCRUDReadsManagedMembershipOnly(t *testing.T) {
	...
	mux.HandleFunc("/management/organizations/DEFAULT/members/membership-123", func(w http.ResponseWriter, r *http.Request) {
	...
	createPlan := orgMemberPlan(t, schemaResp.Schema, OrgMemberModel{
	...
	var createState OrgMemberModel
	...
	wantDeletePaths := []string{"/management/organizations/DEFAULT/members/membership-123"}
	if !reflect.DeepEqual(deletePaths, wantDeletePaths) {
		t.Fatalf("delete paths = %#v, want %#v", deletePaths, wantDeletePaths)
	}
}

func TestOrgMemberReadRemovesMissingMembershipAndDeleteIgnores404(t *testing.T) {
	var methods []string
	...
	mux.HandleFunc("/management/organizations/DEFAULT/members/missing-membership", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodDelete {
	...
	if diags := state.Set(context.Background(), &OrgMemberModel{
...
func TestOrgMemberReportsLifecycleErrors(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/management/auth/token", func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"access_token":"token","token_type":"bearer"}`))
	...
	var schemaResp resource.SchemaResponse
	resourceUnderTest.Schema(context.Background(), resource.SchemaRequest{}, &schemaResp)
	plan := orgMemberPlan(t, schemaResp.Schema, OrgMemberModel{
		MemberID:   types.StringValue("user-123"),
		MemberType: types.StringValue("user"),
		RoleID:     types.StringValue("role-123"),
	})
	...
	state := orgMemberState(t, schemaResp.Schema, OrgMemberModel{
...
func TestOrgMemberDeleteReportsInvalidStateData(t *testing.T) {
	t.Parallel()
...
func TestOrgMemberCreateReportsReadAfterCreateError(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/management/auth/token", func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"access_token":"token","token_type":"bearer"}`))
...
func TestOrgMemberImportRejectsInvalidID(t *testing.T) {
...
func orgMemberState(t *testing.T, schema resourceschema.Schema, model OrgMemberModel) tfsdk.State {
	t.Helper()
