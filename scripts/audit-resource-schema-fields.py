...
AUDITS = [
    ...
    ResourceSchemaAudit(
        ...
        intentionally_unmanaged={
            ...
            "externalId": "the API does not round-trip the planned value; it is API-generated or ignored",
            "loggedAt": "server-managed login timestamp",
            "loginsCount": "server-managed counter",
            "preferredLanguage": "the API accepts but does not round-trip the planned value",
    ...
    ResourceSchemaAudit(
        ...
        intentionally_unmanaged={
            ...
            "createdAt": "server-managed timestamp",
            "credentialsNonExpired": "API credential lifecycle flag preserved from GET/merge",
            "displayName": "the API derives this from firstName and lastName for organization users",
            "enabled": "managed through the dedicated status endpoint",
            "externalId": "the API does not round-trip the planned value",
            "loggedAt": "server-managed login timestamp",
            "loginsCount": "server-managed counter",
            "preferredLanguage": "the API accepts but does not round-trip the planned value",
            "registrationCompleted": "API lifecycle flag preserved from GET/merge",
    ...
    ResourceSchemaAudit(
        name="graviteeam_trust_domain",
        package_dir=ROOT / "internal" / "resources" / "trustdomain",
        schema_name="UpdateTrustDomain",
        api_to_tf={
...
AUDIT_CANDIDATES = [
    ...
    ResourceSchemaCandidate("graviteeam_trust_domain", ROOT / "internal" / "resources" / "trustdomain", "UpdateTrustDomain"),
